...
prod_ranking_model_2 = pickle.load(open(os.path.join(directory,'prod_ranking_model-2.pkl'),'rb'))
cust_prod_ranking_model_2 = pickle.load(open(os.path.join(directory,'cust_prod_ranking_model-2.pkl'),'rb'))
cust_correlation_model_2 = pickle.load(open(os.path.join(directory,'cust_correlation_model-2.pkl'),'rb'))
prod_correlation_model_2 = pickle.load(open(os.path.join(directory,'prod_correlation_model-2.pkl'),'rb'))
...
def html_code_table(prod_df,table_name,file_name,side):
    ...
    table_head_row = '<tr><th>Items/Services Name</th><th>Price (in Rs.)</th></tr>'
...
def most_popular_table():
    most_popular_prods_2 = prod_ranking_model_2.sort_values('Popularity_Rank',ascending=True)[['Product','Rate']].head(10).reset_index(drop=True)
    
    html_code_table(most_popular_prods_2,'Most Popular Items/Services','mostpopulartable','left')

# This function calls the html_code_table function to create a .html file for Top Selling Products
def top_sell_table():
    top_sell_prods_2 = prod_ranking_model_2.sort_values('Top_Sell_Rank',ascending=True)[['Product','Rate']].head(10).reset_index(drop=True)
    
    html_code_table(top_sell_prods_2,'Top Selling Items/Services','topselltable','right')
...
# # Hotels Frequently Purchased and Purchased the Most Products

# In[9]:


# This function calls the html_code_table function to create a .html file for Most Popular Products of a Customer
def cust_most_popular_table_1(cust_name_1):
    cust_most_popular_prods_2 = cust_prod_ranking_model_2[cust_prod_ranking_model_2['Party'] == cust_name_1]
    cust_most_popular_prods_2 = cust_most_popular_prods_2.sort_values('Popularity_Rank',ascending=True)[['Product','Rate']].head(10).reset_index(drop=True)
    
    html_code_table(cust_most_popular_prods_2,'Items/Services  Frequently Opted','custmostpopulartable_1','left')

# This function calls the html_code_table function to create a .html file for Top Selling Products of a Customer
def cust_top_sell_table_1(cust_name_1):
    cust_top_sell_prods_2 = cust_prod_ranking_model_2[cust_prod_ranking_model_2['Party'] == cust_name_1]
    cust_top_sell_prods_2 = cust_top_sell_prods_2.sort_values('Top_Sell_Rank',ascending=True)[['Product','Rate']].head(10).reset_index(drop=True)
    
    #print(cust_top_sell_prods)
    html_code_table(cust_top_sell_prods_2,'Items/Services you Purchased the Most','custtopselltable_1','right')


# In[8]:


# # Products Hotel may Like
...
def recommend_prod_cust_1(cust_name_1):
    similar_custs_corr_2 = cust_correlation_model_2.loc[cust_name_1].sort_values(ascending=False)
    
    prod_by_similar_custs_2 = pd.DataFrame()
    
    # get the products purchased by each customer and multiply with the customer correlation coefficient
    for i in range(len(similar_custs_corr_2)):
        if similar_custs_corr_2.index[i] != cust_name_1:
            cust_top_sell_prods_2 = cust_prod_ranking_model_2[cust_prod_ranking_model_2['Party'] == similar_custs_corr_2.index[i]]
            cust_top_sell_prods_2 = cust_top_sell_prods_2[['Product','Qty','Rate']].reset_index(drop=True)
            cust_top_sell_prods_2['Qty_Corr'] = cust_top_sell_prods_2['Qty'] * similar_custs_corr_2.iloc[i]
            prod_by_similar_custs_2 = pd.concat([cust_top_sell_prods_2,prod_by_similar_custs_2])
    
    # aggregate the Qty Correlation by Product
    prod_by_similar_custs_2 = prod_by_similar_custs_2.groupby('Product').agg({'Qty_Corr':'sum','Rate':'max'})
    prod_by_similar_custs_2.reset_index(inplace=True)
    #print(prod_by_similar_custs.head(20))
    
    # ignore the products already purchased by the input customer
    # merge prod_by_similar_custs and customer purchased products and drop the rows with No_of_orders being Not Null
    input_cust_top_sell_prods_2 = cust_prod_ranking_model_2[cust_prod_ranking_model_2['Party'] == cust_name_1]
    df_merge_2 = pd.merge(prod_by_similar_custs_2,input_cust_top_sell_prods_2[['Product','No_of_Orders']],how='left',on='Product')
    prod_recommend_to_cust_2 = df_merge_2[df_merge_2['No_of_Orders'].isnull()]
    
    # sort the dataframe on Qty_Corr
    prod_recommend_to_cust_2 = prod_recommend_to_cust_2.sort_values('Qty_Corr',ascending=False)[['Product','Rate']].head(10).reset_index(drop=True)
    
    #print(prod_recommend_to_cust)
    
    html_code_table(prod_recommend_to_cust_2,'Products you may like','prodrecommendtable_1','center')
...
def similar_prods_2(prod_name):
    similar_prods_corr_2 = prod_correlation_model_2.loc[prod_name].sort_values(ascending=False)
    
    similar_prods_2 = pd.merge(similar_prods_corr_2,prod_ranking_model_2[['Product','Rate']],how='left',on='Product')
    
    prod_price_2 = similar_prods_2[similar_prods_2['Product'] == prod_name]['Rate'].values[0]
    
    input_prod_index_2 = similar_prods_2[similar_prods_2['Product'] == prod_name].index
    similar_prods_2.drop(index=input_prod_index_2,inplace=True)
    
    similar_prods_2 = similar_prods_2[['Product','Rate']].head(10).reset_index(drop=True)
    
    #print(similar_prods_2)
    
    html_code_table(similar_prods_2,'Customers who purchased this product also purchased these','similarprodtable','left')
...
def search():
    ...
    cust_name_1 = str(request.args.get('name'))
    
    
    if cust_name_1 in cust_prod_ranking_model_2['Party'].unique():
        cust_most_popular_table_1(cust_name_1)
        cust_top_sell_table_1(cust_name_1)
        recommend_prod_cust_1(cust_name_1)
        return render_template('hotel_home.html',name=cust_name_1,new='n')
    else:
        return render_template('hotel_home.html',name=cust_name_1,new='y')
...
def view():
    ...
    if prod_name in prod_ranking_model_2['Product'].unique():
        prod_price_2 = similar_prods_2(prod_name)
        return render_template('prod_view.html',prod=prod_name,price=prod_price_2,exists='y')
    else:
